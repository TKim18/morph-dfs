...
import static org.apache.hadoop.hdfs.protocol.BlockType.*;
...
public class INodeFile extends INodeWithAdditionalFields
  ...
  enum HeaderFormat {
    ...
    ORIGINAL_LAYOUT_AND_REDUNDANCY(STORAGE_POLICY_ID.BITS,
        HeaderFormat.ORIGINAL_SCHEME_BIT_WIDTH, 0);
    ...
    /**
     * Number of bits used to encode original block layout type
     * before the original scheme was transitioned
     */
    private static final int ORIGINAL_SCHEME_BIT_WIDTH = 12;
    ...
    static boolean isStriped(long header) {
      BlockType type = getBlockType(header);
      return type == STRIPED || type == PARITY;
    }
  ...
  private BlockInfo[] parities;
  private short stripeWidth; // TODO: move this into the header
  ...
  INodeFile(long id, byte[] name, PermissionStatus permissions, long mtime,
            long atime, BlockInfo[] blklist, BlockInfo[] parityBlkList,
            Short replication, Byte ecPolicyID, Byte dataStripingID,
            long preferredBlockSize, byte storagePolicyID, BlockType blockType) {
    this(id, name, permissions, mtime, atime, blklist, replication,
      ecPolicyID, preferredBlockSize, storagePolicyID, blockType);
    if (dataStripingID != null) {
      final long dataLayout = HeaderFormat.getBlockLayoutRedundancy(STRIPED, null, dataStripingID);
      header = HeaderFormat.ORIGINAL_LAYOUT_AND_REDUNDANCY.BITS.combine(dataLayout, header);
    }
    setParities(parityBlkList);
  }
  ...
  static String checkBlockComplete(BlockInfo[] blocks, int i,
    ...
    if (!b.isHybrid() && (b.isStriped() || i < blocks.length - numCommittedAllowed)) {
  ...
  public final short getFileReplication() {
    if (isHybrid()) {
      // TODO: repl is stored in the inode object in memory, not in the header
      return replication;
    }
    if (isStriped()) {
      return DEFAULT_REPL_FOR_STRIPED_BLOCKS;
  ...
  public void setHybridFileReplication(short replication) {
  ...
  @Override
  public void setErasureCodingPolicyID(byte id) {
    if (isStriped()) {
      // retrieve the current layout redundancy and store it in the header
      final long currentLayoutRedundancy = HeaderFormat.getECPolicyID(header);
      final long newLayoutRedundancy = HeaderFormat.getBlockLayoutRedundancy(STRIPED, null, id);
      header = HeaderFormat.ORIGINAL_LAYOUT_AND_REDUNDANCY.BITS.combine(currentLayoutRedundancy, header);
      header = HeaderFormat.BLOCK_LAYOUT_AND_REDUNDANCY.BITS.combine(newLayoutRedundancy, header);
    }
  }
  ...
  /** @return the parity block sets of a file */
  public BlockInfo[] getParities() {
    if (this.parities == null) {
      this.setParities(BlockInfo.EMPTY_ARRAY);
    }
  ...
  /**
   * add a parity to the list of parity groups
   */
  void addParity(BlockInfo newblock) {
    Preconditions.checkArgument(newblock.isGrouped());
    if (this.parities.length == 0) {
      this.setParities(new BlockInfo[]{newblock});
    } else {
      int size = this.parities.length;
      BlockInfo[] newlist = new BlockInfo[size + 1];
      System.arraycopy(this.parities, 0, newlist, 0, size);
      newlist[size] = newblock;
      this.setParities(newlist);
    }
  }
  ...
  /** Set the parities */
  @Override
  public void setParities(BlockInfo[] parities) {
    this.parities = (parities != null ? parities : BlockInfo.EMPTY_ARRAY);
  }

  /** Clear all blocks of the file. */
  public void clearBlocks() {
    this.blocks = BlockInfo.EMPTY_ARRAY;
    this.parities = BlockInfo.EMPTY_ARRAY;
  }
  ...
  public void clearFile(ReclaimContext reclaimContext) {
    ...
    if (parities != null && reclaimContext.collectedBlocks != null) {
      for (BlockInfo prty : parities) {
        reclaimContext.collectedBlocks.addDeleteBlock(prty);
      }
    }
    clearBlocks();
       ...
       if (!includesLastUcBlock) {
         ...
         size = isStriped()?
             getPreferredBlockSize() *
                 ((BlockInfoData)lastBlk).getDataBlockNum() :
             getPreferredBlockSize();
       }
    }
    //sum other blocks
    ...
    for (BlockInfo b : blocks) {
      if (b.isHybrid()) {
        ...
        long blockSize = b.isComplete() ?
            ((BlockInfoDecoupled)b).spaceConsumed() : getPreferredBlockSize() *
            ((BlockInfoDecoupled)b).getNumUnits();
        counts.addStorageSpace(blockSize);
      }
    }
    return  counts;
  ...
  @Override
  public BlockInfo getLastParity() {
    return parities.length == 0 ? null : parities[parities.length - 1];
  }
