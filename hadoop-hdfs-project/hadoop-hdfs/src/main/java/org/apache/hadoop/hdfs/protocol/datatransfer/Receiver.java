...
public abstract class Receiver implements DataTransferProtocol {
  ...
  protected final void processOp(Op op) throws IOException {
    ...
    case WRITE_BLOCK:
      opWriteBlock(in);
      break;
    case WRITE_HYBRID_BLOCK:
      opWriteHybridBlock(in);
      break;
    case REPLACE_BLOCK:
      opReplaceBlock(in);
      break;
  ...
  private void opWriteHybridBlock(DataInputStream in) throws IOException {
    final OpWriteHybridBlockProto proto = OpWriteHybridBlockProto.parseFrom(vintPrefixed(in));
    final DatanodeInfo[] replicaNodes = PBHelperClient.convert(proto.getReplicaNodesList());
    final DatanodeInfo[] stripeNodes = PBHelperClient.convert(proto.getStripedNodesList());
    TraceScope traceScope = continueTraceSpan(proto.getHeader(),
            proto.getClass().getSimpleName());
    try {
      writeHybridBlock(
          ...
          // other replica info
          replicaNodes,
          PBHelperClient.convertStorageTypes(proto.getReplicaStorageTypesList(), replicaNodes.length),
          proto.getReplicaStorageIDsList().toArray(new String[0]),
          // striped block info
          PBHelperClient.convert(proto.getStripedBlock()),
          PBHelperClient.convert(proto.getParityBlock()),
          stripeNodes,
          PBHelperClient.convertStorageTypes(proto.getStripedStorageTypesList(), stripeNodes.length),
          proto.getStripedStorageIDsList().toArray(new String[0]),
          proto.getStripedOffsetsList().toArray(new Long[0]),
          PBHelperClient.convertErasureCodingPolicy(proto.getEcPolicy()),
          // write metadata
          PBHelperClient.convert(proto.getSrcDataNode()),
          fromProto(proto.getStage()),
          proto.getPipelineSize(),
