...
class StripedBlockReconstructor extends StripedReconstructor
  ...
  void reconstruct() throws IOException {
    ...
    while (getPositionInBlock() < getMaxTargetLength()) {
      ...
      totalDecodeTime += decodeEnd - readEnd;

      updatePositionInBlock(toReconstructLen);

      clearBuffers();
    }
    LOG.info("Total Time spent decoding: " + totalDecodeTime);
  }
  ...
  private void reconstructTargets(int toReconstructLen) throws IOException {
    ...
    LOG.debug("Reconstructing targets by decoding inputs={}, outputs={}, erased={}",
            inputs.length, outputs.length, Arrays.toString(erasedIndices));
  ...
  private void decode(ByteBuffer[] inputs, int[] erasedIndices,
    ...
    LOG.info("DECODE TIME = " + (end - start)/1000000 + " ms with decoder " + getDecoder().getClass().getSimpleName());
