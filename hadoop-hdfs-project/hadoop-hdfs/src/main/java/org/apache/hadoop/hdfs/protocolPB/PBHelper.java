...
import org.apache.hadoop.hdfs.protocol.proto.DatanodeProtocolProtos.BlockECTranscodeCommandProto;
...
import org.apache.hadoop.hdfs.protocol.proto.ErasureCodingProtos.BlockECTranscodeInfoProto;
import org.apache.hadoop.hdfs.protocol.proto.HdfsProtos;
import org.apache.hadoop.hdfs.protocol.proto.HdfsProtos.BlockProto;
import org.apache.hadoop.hdfs.protocol.proto.HdfsProtos.ExtendedGroupProto;
...
public class PBHelper {
  ...
  public static DatanodeCommand convert(DatanodeCommandProto proto) {
    ...
    case BlockECTranscodeCommand:
      return PBHelper.convert(proto.getBlkECTranscodeCmd());
    default:
      return null;
    }
  }
  ...
  public static DatanodeCommandProto convert(DatanodeCommand datanodeCommand) {
    ...
    case DatanodeProtocol.DNA_ERASURE_CODING_RECONSTRUCTION:
      ...
      break;
    case DatanodeProtocol.DNA_ERASURE_TRANSCODING:
      builder.setCmdType(DatanodeCommandProto.Type.BlockECTranscodeCommand)
        .setBlkECTranscodeCmd(
          convert((BlockECTranscodeCommand) datanodeCommand));
      break;
  ...
  public static BlockECReconstructionInfo convertBlockECReconstructionInfo(
    ...
    ExtendedGroupProto groupProto = blockEcReconstructionInfoProto.getGroup();
    ExtendedGroup group = PBHelperClient.convert(groupProto);
    ...
    return new BlockECReconstructionInfo(group, sourceDnInfos, targetDnInfos,
  ...
  public static BlockECReconstructionInfoProto convertBlockECRecoveryInfo(
    ...
    builder.setGroup(PBHelperClient.convert(
        blockEcRecoveryInfo.getExtendedGroup()));
  ...
  public static BlockECTranscodeInfo convertBlockECTranscodeInfoProto(BlockECTranscodeInfoProto proto) {
    ExtendedGroup group = PBHelperClient.convert(proto.getGroup());
    DatanodeInfo[] dataSources = PBHelperClient.convert(proto.getSourceDnInfos());
    ErasureCodingPolicy ecPolicy = PBHelperClient.convertErasureCodingPolicy(proto.getEcPolicy());
    DatanodeInfo[] targetSources = PBHelperClient.convert(proto.getTargetDnInfos());
    StorageType[] targetStorageTypes = PBHelperClient.convertStorageTypes(
      proto.getTargetStorageTypes().getStorageTypesList(),
      proto.getTargetStorageTypes().getStorageTypesList().size());
    String[] targetStorageUuids = convert(proto.getTargetStorageUuids());
    ExtendedBlock[] parityBlock = PBHelperClient.convertExtendedBlocks(proto.getParityBlocksList());
    DatanodeInfo[] paritySources = null;
    ECSchema oldSchema = PBHelperClient.convertECSchema(proto.getOldSchema());
    if (parityBlock != null) {
      paritySources = PBHelperClient.convert(proto.getParityDnInfos());
    }

    return new BlockECTranscodeInfo(
        group, dataSources, ecPolicy, targetSources,
      targetStorageTypes, targetStorageUuids,
      parityBlock, paritySources, oldSchema);
  }

  public static BlockECTranscodeInfoProto convertBlockECTranscodeInfo(BlockECTranscodeInfo info) {
    BlockECTranscodeInfoProto.Builder builder = BlockECTranscodeInfoProto.newBuilder();
    builder.setGroup(PBHelperClient.convert(info.getGroup()));
    builder.setSourceDnInfos(convertToDnInfosProto(info.getDataSources()));
    builder.setTargetDnInfos(convertToDnInfosProto(info.getTargets()));
    builder.setTargetStorageTypes(convertStorageTypesProto(info.getTargetStorageTypes()));
    builder.setTargetStorageUuids(convertStorageIDs(info.getTargetStorageIds()));
    builder.setEcPolicy(PBHelperClient.convertErasureCodingPolicy(info.getEcPolicy()));
    builder.setOldSchema(PBHelperClient.convertECSchema(info.getOldSchema()));

    if (info.getParityBlocks() != null) {
      builder.addAllParityBlocks(PBHelperClient.convert(info.getParityBlocks()));
      builder.setParityDnInfos(convertToDnInfosProto(info.getParitySources()));
    }
    return builder.build();
  }
  ...
  public static BlockECTranscodeCommandProto convert(
    BlockECTranscodeCommand blkECTranscodeCmd) {
    BlockECTranscodeCommandProto.Builder builder =
      BlockECTranscodeCommandProto.newBuilder();
    Collection<BlockECTranscodeCommand.BlockECTranscodeInfo> infos =
      blkECTranscodeCmd.getTranscodeTasks();
    for (BlockECTranscodeCommand.BlockECTranscodeInfo info : infos) {
      builder.addBlockECTranscodeInfo(convertBlockECTranscodeInfo(info));
    }
    return builder.build();
  }
  public static BlockECTranscodeCommand convert(
    BlockECTranscodeCommandProto blkECTranscodeCmdProto) {
    Collection<BlockECTranscodeInfo> infos = new ArrayList<>();
    List<ErasureCodingProtos.BlockECTranscodeInfoProto> protos =
      blkECTranscodeCmdProto.getBlockECTranscodeInfoList();
    for (ErasureCodingProtos.BlockECTranscodeInfoProto proto : protos) {
      infos.add(convertBlockECTranscodeInfoProto(proto));
    }
    return new BlockECTranscodeCommand(
      DatanodeProtocol.DNA_ERASURE_TRANSCODING,
      infos);
  }
