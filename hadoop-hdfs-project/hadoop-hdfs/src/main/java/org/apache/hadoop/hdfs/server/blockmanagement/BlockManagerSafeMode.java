...
class BlockManagerSafeMode {
  ...
  void activate(long total) {
    ...
    setBlockTotal(total);
    LOG.info("Leaving safe mode, a {}", areThresholdsMet());
    if (areThresholdsMet()) {
  ...
  void checkSafeMode() {
    ...
    case PENDING_THRESHOLD:
      LOG.info("Leaving safe mode, b, are threshold met {}", areThresholdsMet());
      if (areThresholdsMet()) {
  ...
  boolean leaveSafeMode(boolean force) {
    ...
    LOG.info("Leaving safe mode, initializing repl queues");
  ...
  synchronized void incrementSafeBlockCount(int storageNum,
    ...
    LOG.info("Current safe block count = {}", this.blockSafe);
    ...
    if (storageNum >= safeNumberOfNodes) {
      LOG.info("Incrementing safe block count because storage={}, num={}", storageNum, safeNumberOfNodes);
      this.blockSafe++;
  ...
  private void initializeReplQueuesIfNecessary() {
    ...
    LOG.info("If necessary, initializing repl queues");
  ...
  private boolean areThresholdsMet() {
    ...
    synchronized (this) {
      ...
      LOG.info("in threshold met block threshold = {}, data={}",
        isBlockThresholdMet, isDatanodeThresholdMet);
