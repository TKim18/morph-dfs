...
class FSDirStatAndListingOp {
  ...
  static GetBlockLocationsResult getBlockLocations(
    ...
    try {
      ...
      final LocatedBlocks blocks = bm.createLocatedBlocks(
          inode.getBlocks(iip.getPathSnapshotId()), inode.getParities(),
  ...
  private static HdfsFileStatus createFileStatus(
    ...
    boolean isHybridProtected = false;
    ...
    if (node.isFile()) {
      ...
      isHybridProtected = isErasureCoded && fileNode.isHybrid();
      if (isEncrypted) {
      ...
      if (needLocation) {
        ...
        loc = fsd.getBlockManager().createLocatedBlocks(
            fileNode.getBlocks(snapshot), fileNode.getParities(),
            fileSize, isUc, 0L, size,
    ...
    EnumSet<HdfsFileStatus.Flags> flags =
        DFSUtil.getFlags(isEncrypted, isErasureCoded, isHybridProtected, isSnapShottable, hasAcl);
