...
class DataXceiverServer implements Runnable {
  ...
  private final HashMap<ExtendedBlock, ParityHandler> parityHandlers = new HashMap<>();
  ...
  ParityHandler startParityHandler(ExtendedBlock block, ErasureCodingPolicy ecPolicy, int blockSize,
                                   DatanodeInfo[] targets, StorageType[] storageTypes, String[] storageIds) {
    lock.lock();
    try {
      ParityHandler handler = new ParityHandler(
          datanode, block, ecPolicy, blockSize,
          targets, storageTypes, storageIds);
      parityHandlers.put(block, handler);
      new Daemon(datanode.threadGroup, handler).start();
      return handler;
    } catch (Exception e) {
      LOG.error("Failed to initialize parity handler: ", e);
      return null;
    } finally {
      lock.unlock();
    }
  ...
  ParityHandler getParityHandler(ExtendedBlock block) {
    lock.lock();
    try {
      return parityHandlers.get(block);
    } finally {
      lock.unlock();
    }
  }

  int totalParityHandlers() {
    lock.lock();
    try {
      return parityHandlers.size();
    } finally {
      lock.unlock();
    }
  }

  void closeParityHandler(ExtendedBlock block) {
    lock.lock();
    try {
      parityHandlers.remove(block);
    } finally {
      lock.unlock();
    }
  }
