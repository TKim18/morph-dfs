...
public class FSDirectory implements Closeable {
  ...
  public void updateSpaceForCompleteBlock(BlockInfo completeBlk,
    ...
    if (fileINode.isStriped()) {
      ...
      if (completeBlk.isHybrid()) {
        actualBlockGroupSize = completeBlk.getNumBytes();
      } else {
        final BlockInfoDecoupled striped;
        if (completeBlk.isStriped()) {
          striped = new BlockInfoData(completeBlk, numDataUnits, ecPolicy.getCellSize());
        } else {
          striped = new BlockInfoParity(completeBlk, numParityUnits, ecPolicy.getCellSize());
        }
        actualBlockGroupSize = striped.spaceConsumed();
      }
