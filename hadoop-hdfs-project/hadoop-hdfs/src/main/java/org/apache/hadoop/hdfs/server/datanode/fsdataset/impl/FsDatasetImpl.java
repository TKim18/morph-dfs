...
import static org.apache.hadoop.io.nativeio.NativeIO.POSIX.FALLOC_FL_KEEP_SIZE;
import static org.apache.hadoop.io.nativeio.NativeIO.POSIX.FALLOC_FL_PUNCH_HOLE;
...
class FsDatasetImpl implements FsDatasetSpi<FsVolumeImpl> {
  ...
  /** Used to cache partial stripe replicas in memory instead of re-appending */
  final ConcurrentMap<Block, ReplicaInPipeline> cachedReplicasMap;
  ...
  FsDatasetImpl(DataNode datanode, DataStorage storage, Configuration conf
    ...
    cachedReplicasMap = new ConcurrentHashMap<>();
  ...
  private InputStream getBlockInputStreamWithCheckingPmemCache(
    ...
    if (cachePath != null) {
      ...
      LOG.info("Get InputStream by cache file path.");
  ...
  public ReplicaHandler append(ExtendedBlock b,
    ...
    try (AutoCloseableLock lock = datasetWriteLock.acquire()) {
      ...
      boolean inCache = cachedReplicasMap.containsKey(b.getLocalBlock());
      if (!inCache && replicaInfo.getState() != ReplicaState.FINALIZED) {
        throw new ReplicaNotFoundException(
      ...
      try {
        if (inCache) {
          replica = cachedReplicasMap.get(b.getLocalBlock());
        } else {
  ...
  public ReplicaHandler createRbw(
      ...
      boolean allowLazyPersist, boolean allowReplicaCache) throws IOException {
    try (AutoCloseableLock lock = datasetWriteLock.acquire()) {
      ...
      if (allowReplicaCache) {
        cachedReplicasMap.put(b.getLocalBlock(), newReplicaInfo);
      }
  ...
  public void finalizeBlock(ExtendedBlock b, boolean fsyncDir)
    ...
    cachedReplicasMap.remove(b.getLocalBlock());
  ...
  private ReplicaInfo finalizeReplica(String bpid, ReplicaInfo replicaInfo)
    ...
    try (AutoCloseableLock lock = datasetWriteLock.acquire()) {
      ...
      if (volumeMap.get(bpid, replicaInfo.getBlockId()).getGenerationStamp()
        ...
        LOG.warn("Generation stamp should be monotonically increasing.");
...
//            + "increased.");
  ...
  @Override // FsDatasetSpi
  public void punchHoleInFile(
          final ExtendedBlock block,
          final long offset,
          final long numBytes) throws IOException {
    try (AutoCloseableLock lock = datasetWriteLock.acquire()) {
      // create the file output streams by opening it for append
      final String bpid = block.getBlockPoolId();
      final ReplicaInfo replicaInfo = volumeMap.get(bpid, block.getBlockId());
      final FileIoProvider fileIoProvider = datanode.getFileIoProvider();

      LocalReplica replica;
      if (replicaInfo instanceof LocalReplica) {
        // cast
        replica = (LocalReplica) replicaInfo;
      } else {
        LOG.error("Unable to cast replica to local replica");
        return;
      }

      try (RandomAccessFile blockRAF = fileIoProvider.getRandomAccessFile(
              replica.getVolume(), replica.getBlockFile(), "rw")) {
        FileDescriptor fd = blockRAF.getFD();
        LOG.info("Length of the file = {}", blockRAF.length());

        int mode = FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE;
        LOG.info("Punching hole in file with mode = {}, offset = {}, numbytes = {}.", mode, offset, numBytes);
        if (mode != 3) {
          // punch hole is 0x2 and keep size is 0x1 and collapse is 0x8
          LOG.info("Added as insurance, changing mode to 3.");
          mode = 3;
        }
        fileIoProvider.fallocate(replica.getVolume(), fd, mode, offset, numBytes);
        LOG.info("Done fallocating the replica");
      }
    } catch (NativeIOException nie) {
      LOG.error("Unable to execute fallocate.", nie);
    } catch (IOException ioe) {
      LOG.error("Unable to open the replica's output stream for punching.", ioe);
    }
  }
  ...
  public Replica updateReplicaUnderRecovery(
    ...
    try (AutoCloseableLock lock = datasetWriteLock.acquire()) {
      ...
      //check replica state
      if (replica.getState() != ReplicaState.RUR) {
        LOG.info("Replica is actually = {}", replica.getState());
...
//            + ", replica=" + replica);
  ...
  private ReplicaInfo updateReplicaUnderRecovery(
    ...
    //check recovery id
//    if (rur.getRecoveryID() != recoveryId) {
//      LOG.warn("Recovery id comparison = {} vs {}", rur.getRecoveryID(), recoveryId);
...
//    }
    ...
    LOG.info("Copy on truncate = {}, newblockid = {}, currblockid = {}",
            copyOnTruncate, newBlockId, rur.getBlockId());
    if(!copyOnTruncate) {
    ...
    if (rur.getNumBytes() > newlength) {
      if(!copyOnTruncate) {
        LOG.info("Not copying on truncate");
  ...
  @Override // FsDatasetSpi
  public void modifyBlockId(ExtendedBlock block, long newBlockId, long newGenerationStamp) throws IOException {
    String bpid = block.getBlockPoolId();
    long oldBlockId = block.getBlockId();
    ReplicaInfo replicaInfo = getReplicaInfo(bpid, oldBlockId);

    try (AutoCloseableLock lock = datasetReadLock.acquire()) {
      LOG.info("Modifying block id = {} to {} with gs {}",
              block, newBlockId, newGenerationStamp);
      // pop the replica to modify
      LOG.info("Replica info = {}", replicaInfo);

      // retrieve the current block directory
      final String parent = new File(replicaInfo.getBlockURI()).getParent();
      LOG.info("Getting parent = {}", parent);

      // add replica back at the same level with the new ID
      replicaInfo.setBlockId(newBlockId);
      String newBlockName = replicaInfo.getBlockName();
      replicaInfo.setBlockId(oldBlockId);
      // retrieve new block name and swap back

      LOG.info("Getting files");
      final File newDataName = new File(parent, newBlockName);
      final File newMetaName = FsDatasetUtil.getMetaFile(newDataName, newGenerationStamp);
      LOG.info("Parent directory = {}, newDataName = {}, newMetaName = {}",
          parent, newDataName.getPath(), newMetaName.getPath());
      try {
        replicaInfo.renameMeta(newMetaName.toURI());
      } catch (IOException e) {
        throw new IOException("Failed to move meta file for " + block
                + " from " + replicaInfo.getMetadataURI() + " to " + newMetaName, e);
      }
      try {
        replicaInfo.renameData(newDataName.toURI());
      } catch (IOException e) {
        throw new IOException("Failed to move block file for " + block
                + " from " + replicaInfo.getBlockURI() + " to "
                + newDataName.getAbsolutePath(), e);
      }
    }

    LOG.info("Modifiying volume");
    volumeMap.remove(bpid, replicaInfo);
    replicaInfo.setBlockId(newBlockId);
    replicaInfo.setGenerationStamp(newGenerationStamp);
    volumeMap.add(bpid, replicaInfo);
  }
  ...
  public void onCompleteLazyPersist(String bpId, long blockId,
    ...
    try (AutoCloseableLock lock = datasetWriteLock.acquire()) {
      ...
      LOG.info("LazyWriter: Finish persisting RamDisk block: "
