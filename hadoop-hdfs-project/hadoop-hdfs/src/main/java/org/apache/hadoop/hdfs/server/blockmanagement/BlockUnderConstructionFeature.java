...
public class BlockUnderConstructionFeature {
  ...
  public void setExpectedLocations(Block block, DatanodeStorageInfo[] targets,
    ...
    for(int i = 0; i < targets.length; i++) {
      if (targets[i] != null) {
        ...
        Block replicaBlock =
            blockType == BlockType.STRIPED || blockType == BlockType.PARITY || blockType == BlockType.HYBRID ?
  ...
  /**
   * when committing a data stripe whose size is less than a stripe, we need
   * to decrease the scheduled block size of the DataNodes that do not store
   * any internal block.
   */
  void updateStorageScheduledSize(BlockInfoData storedBlock) {
