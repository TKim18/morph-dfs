...
import org.apache.hadoop.hdfs.server.blockmanagement.BlockInfoDecoupled;
    ...
    if (isComplete) {
      if (isStriped) {
        ...
        Iterable<StorageAndBlockIndex> sis =
            ((BlockInfoDecoupled) storedBlock).getStorageAndIndexInfos();
  ...
  private void collectBlocksSummary(String parent, HdfsFileStatus file,
    ...
    for (LocatedBlock lBlk : blocks.getLocatedBlocks()) {
      ...
      if (file.getErasureCodingPolicy() != null) {
        assert storedBlock instanceof BlockInfoDecoupled;
        targetFileReplication = ((BlockInfoDecoupled) storedBlock).getNumUnits();
      } else {
    ...
    if (doReplicate && !misReplicatedBlocks.isEmpty()) {
      LOG.info("Fsck: Dealing with block report?");
