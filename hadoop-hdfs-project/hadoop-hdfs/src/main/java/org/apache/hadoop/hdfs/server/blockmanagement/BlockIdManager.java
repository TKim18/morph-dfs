...
public class BlockIdManager {
  ...
  public static boolean isGroupedBlockID(long id) {
    return BlockType.isGroupedBlockId(id);
  }
  ...
  /**
   * The same method as convertToStripedID as the original method just
   * eliminates the 4 least significant bits from the ID which is
   * exactly what a parity block ID needs to be converted to a group ID.
   */
  static long convertToGroupID(long id) {
    return convertToStripedID(id);
  }
