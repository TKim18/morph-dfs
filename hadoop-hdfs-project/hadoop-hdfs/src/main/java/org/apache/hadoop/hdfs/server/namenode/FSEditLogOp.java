...
public abstract class FSEditLogOp {
  ...
  /**
   * Operation corresponding to change erasure coding policy
   */
  static class ModifyErasureCodingPolicyOp extends FSEditLogOp {

    String path;
    Byte ecPolicyId;

    ModifyErasureCodingPolicyOp() {
      // TODO: implement what the edit log op should look like
      super(OP_MODIFY_ERASURE_CODING_POLICY);
    }

    static ModifyErasureCodingPolicyOp getInstance(OpInstanceCache cache) {
      return (ModifyErasureCodingPolicyOp) cache.get(OP_MODIFY_ERASURE_CODING_POLICY);
    }
    ...
    public void writeFields(DataOutputStream out) throws IOException {
      FSImageSerialization.writeString(path, out);
      FSImageSerialization.writeByte(ecPolicyId, out);
    }

    @Override
    void readFields(DataInputStream in, int logVersion) throws IOException {
      path = FSImageSerialization.readString(in);
      ecPolicyId = FSImageSerialization.readByte(in);
    }

    @Override
    public String toString() {
      StringBuilder sb = new StringBuilder();
      sb.append("ModifyErasureCodePolicy [path=")
        .append(path)
        .append(", policy=")
        .append(ecPolicyId)
        .append("]");
    ...
    protected void toXml(ContentHandler contentHandler) throws SAXException {
      XMLUtils.addSaxString(contentHandler, "PATH", path);
      XMLUtils.addSaxString(contentHandler, "POLICY", String.valueOf(ecPolicyId));
    }

    @Override
    void fromXml(Stanza st) throws InvalidXmlException {
      this.path = st.getValue("PATH");
      this.ecPolicyId = Byte.parseByte(st.getValue("POLICY"));
    }
  }
