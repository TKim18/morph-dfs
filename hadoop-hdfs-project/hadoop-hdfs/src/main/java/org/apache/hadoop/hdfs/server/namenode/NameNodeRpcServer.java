...
public class NameNodeRpcServer implements NamenodeProtocols {
  ...
  public LocatedBlocks getBlockLocations(String src, 
                                          ...
                                          long length) 
      throws IOException {
    LOG.info("Calling get block locations");
    checkNNStartup();
  ...
  public LocatedBlock addHybridBlock(String src, String clientName,
       ...
       ExtendedBlock previousReplica, boolean parityEnabled,
       DatanodeInfo[] excludedNodes, long fileId,
       String[] favoredNodes, EnumSet<AddBlockFlag> addBlockFlags)
      throws IOException {
    checkNNStartup();
    LocatedBlock locatedBlock = namesystem.getAdditionalHybridBlock(
        src, fileId, clientName, previous, previousParity,
        previousReplica, parityEnabled, excludedNodes, favoredNodes, addBlockFlags);
    if (locatedBlock != null) {
      metrics.incrAddBlockOps();
    }
    return locatedBlock;
  ...
  public LocatedBlock[] addBlocks(String src, String clientName,
    ...
    throws IOException {
    checkNNStartup();
    LocatedBlock[] locatedBlocks = namesystem.getAdditionalBlocks(src, fileId,
      clientName, previousData, previousParities, partialParity,
      excludedNodes, favoredNodes, addBlockFlags, numStripes, numGroups);
    if (locatedBlocks != null) {
      for (int i = 0; i < numStripes + numGroups; i++) {
        metrics.incrAddBlockOps();
      }
    }
    return locatedBlocks;
  ...
  public boolean complete(String src, String clientName,
          ...
          throws IOException {
    checkNNStartup();
    return namesystem.completeFile(src, clientName, last, parities, fileId);
  }
  ...
  public void setStripeWidth(String src, int stripeWidth)
    throws IOException {
    namesystem.setStripeWidth(src, stripeWidth);
  }
