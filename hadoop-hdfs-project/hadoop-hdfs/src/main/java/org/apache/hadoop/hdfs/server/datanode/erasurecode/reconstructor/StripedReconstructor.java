 ...
 */
package org.apache.hadoop.hdfs.server.datanode.erasurecode.reconstructor;
...
public abstract class StripedReconstructor {
  ...
  private final long parityBlockLength;
  ...
  StripedReconstructor(ErasureCodingWorker worker,
    ...
    positionInBlock = 0L;
    blockOffsets = stripedReconInfo.getBlockIdOffsets();
    blockLengths = stripedReconInfo.getBlockLengths();
    parityBlockLength = getMaxBlockLength();
    LOG.info("Orig = {}, Block ID offsets: {}", blockGroup.getBlockId(), Arrays.toString(blockOffsets));
  ...
  ExtendedBlock getBlock(int i) {
    ExtendedBlock b = StripedBlockUtil.constructInternalBlock(blockGroup, ecPolicy, i);
    b.setBlockId(blockGroup.getBlockId() + blockOffsets[i]);
    return b;
  }

  long getMaxBlockLength() {
    long m = 0L;
    for (long l : blockLengths) {
      m = Math.max(m, l);
    }
    return m;
  }
  ...
  long getBlockLen(int i) {
    // block lengths only has data, parity length will always be the max of all data lengths
    return i < ecPolicy.getNumDataUnits() ? blockLengths[i] : parityBlockLength;
  }
  ...
  protected void initEncoder() {
    if (encoder == null) {
      encoder = CodecUtil.createRawEncoder(conf, ecPolicy.getCodecName(), coderOptions);
    }
  }
