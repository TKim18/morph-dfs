...
class LowRedundancyBlocks implements Iterable<BlockInfo> {
  ...
  private int getPriority(BlockInfo block,
    ...
    if (block.isStriped()) {
      if (!block.isGrouped()) {
        BlockInfoData sblk = (BlockInfoData) block;
        return getPriorityStriped(curReplicas, outOfServiceReplicas,
            sblk.getRealDataBlockNum(), (short) 0);
      } else {
        BlockInfoParity sblk = (BlockInfoParity) block;
        return getPriorityStriped(curReplicas, outOfServiceReplicas,
            (short) 0, sblk.getParityBlockNum());
      }
    } else if (block.isGrouped()) {
      BlockInfoGrouped sblk = (BlockInfoGrouped) block;
      return getPriorityStriped(curReplicas, outOfServiceReplicas,
          sblk.getDataBlockNum(), sblk.getParityBlockNum());
    } else {
  ...
  synchronized boolean add(BlockInfo block,
    ...
    if(add(block, priLevel, expectedReplicas)) {
      NameNode.blockStateChangeLog.info(
          ...
          block.getBlockId(), curReplicas, expectedReplicas, priLevel);
      return true;
    }
    return false;
  }
...
//    if(NameNode.stateChangeLog.isDebugEnabled()) {
    NameNode.stateChangeLog.info("LowRedundancyBlocks.update " +
      block +
...
//    }
    ...
    boolean found = remove(block, oldPri, oldExpectedReplicas);
    if (found) {
      NameNode.blockStateChangeLog.info("Unable to find a block to remove from the queue");
    }
  ...
  synchronized List<List<BlockInfo>> chooseLowRedundancyBlocks(
    ...
    for (; count < blocksToProcess && priority < LEVEL; priority++) {
      ...
      for(; count < blocksToProcess && i.hasNext(); count++) {
        ...
        if (block.isDeleted()) {
          NameNode.blockStateChangeLog.info("Block is deleted, file is invalid: {}, {}",
            block, block.getBlockCollectionId());
          toRemove.add(block);
          continue;
        }
        if (!inCorruptLevel) {
