...
class ErasureCodingWork extends BlockReconstructionWork {
  ...
  public ErasureCodingWork(String blockPoolId, BlockInfo block,
    ...
    this(blockPoolId, block, bc, srcNodes, containingNodes, liveReplicaStorages, additionalReplRequired,
            priority, liveBlockIndicies, liveBusyBlockIndicies, null, null);
  }
  ...
  public ErasureCodingWork(String blockPoolId, BlockInfo block,
     ...
     int additionalReplRequired, int priority,
     byte[] liveBlockIndicies, byte[] liveBusyBlockIndicies,
     int[] blockOffsets, long[] blockLengths) {
  ...
  void addTaskToDatanode(NumberReplicas numberReplicas) {
    ...
    BlockInfoGrouped stripedBlk = (BlockInfoGrouped) getBlock();
    ...
    if (hasNotEnoughRack()) {
      ...
      targets[0].getDatanodeDescriptor().addBlockToBeErasureCoded(
          new ExtendedGroup(new ExtendedBlock(blockPoolId, stripedBlk), blockOffsets, blockLengths),
        getSrcNodes(), targets, getLiveBlockIndicies(), stripedBlk.getErasureCodingPolicy());
    }
  }
