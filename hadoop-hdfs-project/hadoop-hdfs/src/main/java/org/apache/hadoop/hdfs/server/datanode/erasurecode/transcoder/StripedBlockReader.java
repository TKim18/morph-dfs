...
class StripedBlockReader {
  ...
  private final StripedReader reader;
  ...
  StripedBlockReader(StripedReader stripedReader, DataNode datanode, Configuration conf,
                     short index, ExtendedBlock block,
                     DatanodeInfo source, long offsetInBlock,
                     CachingStrategy cachingStrategy) {
    this.reader = stripedReader;
  ...
  ByteBuffer getReadBuffer() {
    if (buffer == null) {
      this.buffer = reader.allocateReadBuffer();
    }
    return buffer;
  }
  ...
  private BlockReader createBlockReader(long offsetInBlock) {
    LOG.debug("[BWOCC]-t StripedBlockReader.createBlockReader() offsetInBlock = {}, numBytes = {}", offsetInBlock, block.getNumBytes());
    if (offsetInBlock >= block.getNumBytes()) {
      return null;
    }
    Peer peer = null;
    try {
      InetSocketAddress dnAddr = getSocketAddress4Transfer(source);
  ...
  private BlockReadStats actualReadFromBlock() throws IOException {
    ...
    while (n < len) {
      int nread;
      nread = blockReader.read(buffer);
      if (nread <= 0) {
        break;
      }
      n += nread;
    }
    LOG.info("[BWOCC]-t actualReadFromBlock() read {} bytes", n);
