...
class LocatedBlockBuilder {
  ...
  protected List<LocatedBlock> parities = Collections.<LocatedBlock>emptyList();
  ...
  LocatedBlockBuilder addParity(LocatedBlock parity) {
    if (parities.isEmpty()) {
      parities = new ArrayList<>();
    }
    parities.add(parity);
    return this;
  }
  ...
  LocatedBlocks build() {
    return new LocatedBlocks(flen, isUC, blocks, parities, last,
