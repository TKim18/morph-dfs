...
class StripedReader {
  ...
  StripedReader(StripedReconstructor reconstructor, DataNode datanode,
    ...
    LOG.info("Striped read buffer size for reconstruction: {}", stripedReadBufferSize);
  ...
  StripedBlockReader createReader(int idxInSources, long offsetInBlock) {
    LOG.info("Creating reader at index: {}, liveIndices: {}, liveIndices[indxinsources]: {}, sources: {}",
      idxInSources, liveIndices, liveIndices[idxInSources], sources);
    StripedBlockReader r = new StripedBlockReader(this, datanode,
    ...
    while (!futures.isEmpty()) {
      try {
        ...
        if (result.state == StripingChunkReadResult.SUCCESSFUL) {
          ...
          LOG.debug("Size of failed buffer = {} at index = {}",
            readers.get(resultIndex).getReadBuffer().position(), resultIndex);
