...
public interface FsDatasetSpi<V extends FsVolumeSpi> extends FSDatasetMBean {
   ...
   *
   * @param b                 block
   * @param allowReplicaCache
   * @return the meta info of the replica which is being written to
   * @throws IOException if an error occurs
   */
  ReplicaHandler createRbw(StorageType storageType, String storageId,
                           ExtendedBlock b, boolean allowLazyPersist, boolean allowReplicaCache) throws IOException;
  ...
  /**
   * Punch a hole in the replica's logical sequence using fallocate
   * @return the ID of storage that stores the block
   */
  void punchHoleInFile(
          ExtendedBlock block,
          long offset,
  ...
  /**
   * Modify block and metadata file to reflect the new blockID
   * @param block Current block
   * @param newBlockId New block ID
   */
  void modifyBlockId(ExtendedBlock block, long newBlockId, long newGenerationStamp) throws IOException;
