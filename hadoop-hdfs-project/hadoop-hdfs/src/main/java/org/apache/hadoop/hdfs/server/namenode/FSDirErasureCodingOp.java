...
final class FSDirErasureCodingOp {
  ...
  static FileStatus setErasureCodingPolicy(final FSNamesystem fsn,
    ...
    fsd.writeLock();
    try {
      NameNode.stateChangeLog.info("Setting erasure coding policy on path: {} with policy: {}", srcArg, ecPolicyName);
      ...
      NameNode.stateChangeLog.info("Completed erasure coding policy change for path {}", srcArg);
    } finally {
      fsd.writeUnlock();
    }
  ...
  static void setStripeWidth(final FSNamesystem fsn,
                             final String srcArg, final int stripeWidth,
    ...
    try {
      ...
      NameNode.stateChangeLog.info("Setting stripe width on path: {} with width: {}", srcArg, stripeWidth);
      iip.getLastINode().asFile().setStripeWidth((short) stripeWidth);
    } finally {
      fsd.writeUnlock();
    }
  }
  ...
  private static List<XAttr> setErasureCodingPolicyXAttrDir(final FSNamesystem fsn, FSDirectory fsd,
      final INodesInPath srcIIP, ErasureCodingPolicy ecPolicy, final INode inode) throws IOException {
    final XAttr ecXAttr;
  ...
  public static void unprotectedTransitionFromHybridToEC(
      final FSNamesystem fsn, final INodesInPath srcIIP, final ErasureCodingPolicy ecPolicy) {
    INodeFile file = srcIIP.getLastINode().asFile();
    FSDirectory fsd = fsn.getFSDirectory();
    BlockManager bm = fsd.getBlockManager();

    if (ecPolicy.getNumDataUnits() == 2) {
      // for these, just remove number of replicas by 1
      // figure out how to decrease block replication such that it doesn't mess with the system
      NameNode.stateChangeLog.info("Changing hybrid file={} to hybrid 1 copy", file.toDetailString());

      file.setHybridFileReplication((short) 1);
      for (BlockInfo block : file.getBlocks()) {
        BlockInfoHybrid hybrid = (BlockInfoHybrid) block;
        for (BlockInfo replica : hybrid.getReplicas()) {
          bm.setReplication(replica.getReplication(),
              (short) (replica.getReplication() - 1), replica);
        }
        hybrid.setReplication((short) 1);
      }
    } else {
      // for these, do a full transition to EC from hybrid
      NameNode.stateChangeLog.info("Changing hybrid file={} to just ec", file.toDetailString());

      // Two things must be performed here:
      // 1. change hybrid groups to become data stripes and parity groups
      // 2. enqueue replicated blocks to be deleted
      file.setHybrid(false);
      file.setHybridFileReplication((short) 1);

      BlockInfo[] blocks = file.getBlocks();
      int numBlocks = blocks.length;
      BlockInfo[] dataBlocks = new BlockInfo[numBlocks];
      BlockInfo[] parityGroups = new BlockInfo[numBlocks];
      for (int i = 0; i < numBlocks; i++) {
        BlockInfoHybrid hybridBlock = (BlockInfoHybrid) blocks[i];
        short numData = hybridBlock.getNumDataUnits();
        int cellSize = hybridBlock.getCellSize();

        // set the new data blocks
        dataBlocks[i] = new BlockInfoData(hybridBlock, numData, cellSize);
        System.arraycopy(hybridBlock.storages, 0, dataBlocks[i].storages, 0, numData);
        bm.addBlockCollection(dataBlocks[i], file);

        if (file.getStripeWidth() == 0) {
          file.setStripeWidth(numData);
        }

        // set the new parity groups
        parityGroups[i] = hybridBlock.getParity();

        // enqueue replicas for deletion
        for (BlockInfo replica : hybridBlock.getReplicas()) {
          replica.delete();
          bm.removeBlock(replica);
        }
      }
      file.setBlocks(dataBlocks);
      file.setParities(parityGroups);
    }
  }

  public static void unprotectedSetFileErasureCodingPolicy(final FSNamesystem fsn,
      final INodesInPath srcIIP, ErasureCodingPolicy newPolicy, boolean nonEdit) throws IOException {
    INodeFile file = srcIIP.getLastINode().asFile();
    long inodeId = file.getId();
    FSDirectory fsd = fsn.getFSDirectory();
    BlockManager bm = fsd.getBlockManager();
    NameNode.stateChangeLog.info("Changing file={} to ecpolicy={}", file.toDetailString(), newPolicy);

    // sets the new and old policies TODO: fix this to be cleaner
    ErasureCodingPolicy oldPolicy = getErasureCodingPolicyForPath(fsd, srcIIP);
    if (oldPolicy == null) {
      return;
    }
    file.setErasureCodingPolicyID(newPolicy.getId());

    BlockInfo[] stripes  = file.getBlocks();
    BlockInfo[] parities = file.getParities();

    // determine new groupings
    TranscodeInfo[] newInfos = FSDirDecouplingOp.computeTranscodeOps(fsn, stripes, parities, oldPolicy, newPolicy);

    // add new parities
    BlockInfoParity[] newParities = FSDirDecouplingOp.toParityBlocks(newInfos);
    for (BlockInfo parity : newParities) {
      bm.addBlockCollection(parity, file);
    }

    // kick off new parity construction
    for (TranscodeInfo info : newInfos) {
      info.getGroup().setBlockCollectionId(inodeId);
      if (nonEdit) {
        // TODO: use transcode info instead of new groups
         bm.addTranscodingOperation(info);
      }
    }
    // bm.printBlockInfoState();
  }

  /**
   * Persist the set ec action
   */
  private static void persistErasureCodingPolicyChange(
    FSNamesystem fsn, String path, ErasureCodingPolicy ecPolicy) {
    fsn.getEditLog().logModifyErasureCodingPolicy(path, ecPolicy);
    // TODO: set this to debug only after testing
    if (NameNode.stateChangeLog.isDebugEnabled() || true) {
      NameNode.stateChangeLog.info("persistErasureCodingPolicyChange: "
        + path + " with new policy " + ecPolicy);
    }
  }
  ...
  private static List<XAttr> setErasureCodingPolicyXAttr(final FSNamesystem fsn,
    ...
    if (inode.isDirectory()) {
      return setErasureCodingPolicyXAttrDir(fsn, fsd, srcIIP, ecPolicy, inode);
    } else {
      if (inode.asFile().isHybrid()) {
        unprotectedTransitionFromHybridToEC(fsn, srcIIP, ecPolicy);
      } else {
        unprotectedSetFileErasureCodingPolicy(fsn, srcIIP, ecPolicy, true);
        persistErasureCodingPolicyChange(fsn, srcIIP.getPath(), ecPolicy);
      }
      ...
      for (int i = iip.length() - 1; i >= 0; i--) {
        ...
        if (inode.isFile()) {
          ...
          if (id < 0) {
            ...
            ErasureCodingPolicy policy = fsd.getFSNamesystem().getErasureCodingPolicyManager().getByID(id);
            if (policy != null) {
              policy.setStripeWidth(inode.asFile().getStripeWidth());
            }
            return policy;
          }
        }
