...
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.AddHybridBlockResponseProto;
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.AddBlocksRequestProto;
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.AddBlocksResponseProto;
...
public class ClientNamenodeProtocolServerSideTranslatorPB implements
  ...
  @Override
  public AddHybridBlockResponseProto addHybridBlock(RpcController controller,
                                        AddHybridBlockRequestProto req) throws ServiceException {
    ...
    try {
      ...
      EnumSet<AddBlockFlag> flags =
          PBHelperClient.convertAddBlockFlags(req.getFlagsList());
      LocatedBlock result = server.addHybridBlock(
          req.getSrc(),
          req.getClientName(),
          req.hasPrevious() ? PBHelperClient.convert(req.getPrevious()) : null,
          req.hasPreviousParity() ? PBHelperClient.convert(req.getPreviousParity()) : null,
          req.hasPreviousReplica() ? PBHelperClient.convert(req.getPreviousReplica()) : null,
          req.getParityEnabled(),
          (excl == null || excl.size() == 0) ? null : PBHelperClient.convert(excl
              .toArray(new DatanodeInfoProto[excl.size()])), req.getFileId(),
          (favor == null || favor.size() == 0) ? null : favor
              .toArray(new String[favor.size()]),
          flags);
      return AddHybridBlockResponseProto.newBuilder()
          .setBlock(PBHelperClient.convertLocatedBlock(result)).build();
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  ...
  @Override
  public AddBlocksResponseProto addBlocks(RpcController controller,
                                          AddBlocksRequestProto req) throws ServiceException {
    try {
      List<DatanodeInfoProto> excl = req.getExcludeNodesList();
      List<String> favor = req.getFavoredNodesList();
      List<ExtendedBlockProto> ebs = req.getPreviousParitiesList();
      EnumSet<AddBlockFlag> flags =
              PBHelperClient.convertAddBlockFlags(req.getFlagsList());
      LocatedBlock[] result = server.addBlocks(
              req.getSrc(),
              req.getClientName(),
              req.hasPreviousData() ? PBHelperClient.convert(req.getPreviousData()) : null,
              (ebs == null || ebs.size() == 0) ? null : PBHelperClient.convertExtendedBlocks(ebs),
              req.hasPartialParity() ? PBHelperClient.convert(req.getPartialParity()) : null,
              (excl == null || excl.size() == 0) ? null : PBHelperClient.convert(excl
                      .toArray(new DatanodeInfoProto[excl.size()])), req.getFileId(),
              (favor == null || favor.size() == 0) ? null : favor
                      .toArray(new String[favor.size()]),
              flags, req.getNumStripes(), req.getNumGroups());
      return AddBlocksResponseProto.newBuilder().addAllBlock(Arrays.asList(
              PBHelperClient.convertLocatedBlocks(result))).build();
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  }
  ...
  public CompleteResponseProto complete(RpcController controller,
    ...
    try {
      List<ExtendedBlockProto> parities = req.getParitiesList();
      boolean result = 
          server.complete(req.getSrc(), req.getClientName(),
          req.hasLast() ? PBHelperClient.convert(req.getLast()) : null,
          (parities == null || parities.isEmpty()) ? null : PBHelperClient.convertExtendedBlocks(parities),
  ...
  @Override
  public SetStripeWidthResponseProto setStripeWidth(
    RpcController controller, SetStripeWidthRequestProto req)
    throws ServiceException {
    try {
      server.setStripeWidth(req.getSrc(), req.getStripeWidth());
      return SetStripeWidthResponseProto.newBuilder().build();
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  }
