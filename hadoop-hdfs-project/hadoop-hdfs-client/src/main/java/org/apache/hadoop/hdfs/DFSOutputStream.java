...
public class DFSOutputStream extends FSOutputSummer
  ...
  protected DFSPacket parityPacket = null;
  protected DataStreamer streamer;
  protected DataStreamer pstreamer;
  ...
  protected short blockReplication; // replication factor of file
  ...
  protected DFSPacket createPacket(int packetSize, int chunksPerPkt,
                                   long offsetInBlock, long seqno,
                                   boolean lastPacketInBlock, boolean lastPacketInStripe)
    ...
    return new DFSPacket(buf, chunksPerPkt, offsetInBlock, seqno,
        getChecksumSize(), lastPacketInBlock, lastPacketInStripe);
  }
  ...
  private DFSOutputStream(DFSClient dfsClient, String src,
    ...
    this.cellSize = (int) this.blockSize;
  ...
  static DFSOutputStream newStreamForCreate(DFSClient dfsClient, String src,
    ...
    try (TraceScope ignored =
      ...
      if (stat.isHybridProtected()) {
        out = new DFSHybridOutputStream(dfsClient, src, stat,
            flag, progress, checksum, favoredNodes);
      } else if(stat.getErasureCodingPolicy() != null) {
  ...
  protected synchronized void writeChunk(byte[] b, int offset, int len,
    ...
    if (currentPacket.getNumChunks() == currentPacket.getMaxChunks() ||
        (getStreamer().getBytesCurBlock() != 0 && getStreamer().getBytesCurBlock() % cellSize == 0) || // break up packet at chunk boundary for hybrid writes
  ...
  */
  ...
  protected void writeParityChunk(ByteBuffer buffer, int len,
      byte[] checksum, int ckoff, int cklen) throws IOException {
    writeParityChunkPrepare(len, ckoff, cklen);

    parityPacket.writeChecksum(checksum, ckoff, cklen);
    parityPacket.writeData(buffer, len);
    parityPacket.incNumChunks();
    pstreamer.incBytesCurBlock(len);

    // If packet is full, enqueue it for transmission
    if (parityPacket.getNumChunks() == parityPacket.getMaxChunks() ||
            pstreamer.getBytesCurBlock() == blockSize) {
      enqueueParityPacketFull(pstreamer);
    }
  }
  ...
  private synchronized void writeChunkPrepare(int buflen,
    ...
    if (currentPacket == null) {
      currentPacket = createPacket(packetSize, chunksPerPacket, getStreamer()
          .getBytesCurBlock(), getStreamer().getAndIncCurrentSeqno(), false, false);
  ...
  private void writeParityChunkPrepare(int buflen,
      int ckoff, int cklen) throws IOException {
    dfsClient.checkOpen();
    checkClosed();
    ...
    if (parityPacket == null) {
      parityPacket = createPacket(packetSize, chunksPerPacket, pstreamer
              .getBytesCurBlock(), pstreamer.getAndIncCurrentSeqno(), false, false);
      DFSClient.LOG.debug("WriteChunk allocating new packet seqno={},"
                      + " src={}, packetSize={}, chunksPerPacket={}, bytesCurBlock={}",
              parityPacket.getSeqno(), src, packetSize, chunksPerPacket,
              pstreamer.getBytesCurBlock() + ", " + this);
    }
  }
  ...
  void enqueueParityPacket() throws IOException {
    pstreamer.waitAndQueuePacket(parityPacket);
    parityPacket = null;
  }
  ...
  synchronized void enqueueCurrentPacketFull() throws IOException {
    LOG.debug("enqueue full {}, src={}, bytesCurBlock={}, blockSize={},"
            ...
            .getBytesCurBlock(), blockSize, getStreamer().getAppendChunk(), getStreamer());
    enqueueCurrentPacket();
    adjustChunkBoundary(getStreamer());
    endBlock();
  }

  void enqueueParityPacketFull(DataStreamer streamer) throws IOException {
    LOG.debug("enqueue parity full {}, src={}, bytesCurBlock={}, blockSize={},"
            + " appendChunk={}, {}", parityPacket, src, streamer
            .getBytesCurBlock(), blockSize, streamer.getAppendChunk(), streamer);
    enqueueParityPacket();
  ...
  void setCurrentPacketToEmpty() throws InterruptedIOException {
    currentPacket = createPacket(0, 0, getStreamer().getBytesCurBlock(),
        getStreamer().getAndIncCurrentSeqno(), true, true);
    currentPacket.setSyncBlock(shouldSyncBlock);
  }

  void setCurrentPacketToEmptyWithContinue() throws InterruptedIOException {
    currentPacket = createPacket(0, 0, getStreamer().getBytesCurBlock(),
        getStreamer().getAndIncCurrentSeqno(), true, false);
    currentPacket.setSyncBlock(shouldSyncBlock);
  }

  void setParityPacketToEmpty() throws InterruptedIOException {
    parityPacket = createPacket(0, 0, pstreamer.getBytesCurBlock(),
            pstreamer.getAndIncCurrentSeqno(), true, true);
    parityPacket.setSyncBlock(shouldSyncBlock);
  }
   ...
   */
  protected void adjustChunkBoundary(DataStreamer streamer) {
    ...
    if (!streamer.getAppendChunk()) {
      final int psize = (int) Math
          .min(blockSize - streamer.getBytesCurBlock(), writePacketSize);
  ...
  void endBlock() throws IOException {
    if (getStreamer().getBytesCurBlock() == blockSize) {
      LOG.debug("Sending out final end stripe block from client");
  ...
  void endReplicaBlock() throws IOException {
    if (getStreamer().getBytesCurBlock() == blockSize) {
      LOG.debug("Sending out final end replica block from client");
      setCurrentPacketToEmptyWithContinue();
      enqueueCurrentPacket();
      getStreamer().setBytesCurBlock(0);
      lastFlushOffset = 0;
    }
  ...
  void endParityBlock() throws IOException {
    if (pstreamer.getBytesCurBlock() == blockSize) {
      // create empty packet
      parityPacket = createPacket(0, 0, pstreamer.getBytesCurBlock(),
              pstreamer.getAndIncCurrentSeqno(), true, true);
      parityPacket.setSyncBlock(shouldSyncBlock);
          ...
          if (isSync && currentPacket == null && !endBlock) {
            ...
            currentPacket = createPacket(packetSize, chunksPerPacket,
                getStreamer().getBytesCurBlock(), getStreamer()
                    .getAndIncCurrentSeqno(), false, false);
          }
        } else {
  ...
  protected void flushParity() throws IOException {
    long toWaitFor = flushInternalParityWithoutWaitingAck();
    pstreamer.waitForAckedSeqno(toWaitFor);
  }
  ...
  void completeFile() throws IOException {
    ...
    try (TraceScope ignored =
      ...
      completeFile(lastBlock, new ExtendedBlock[]{});
    }
  }
  ...
  protected void completeFile(ExtendedBlock last, ExtendedBlock[] parities) throws IOException {
    ...
    while (!fileComplete) {
      fileComplete =
          dfsClient.namenode.complete(src, dfsClient.clientName, last, parities, fileId);
      if (!fileComplete) {
...
//          }
  ...
  protected long flushInternalParityWithoutWaitingAck() throws IOException {
    dfsClient.checkOpen();
    checkClosed();
    pstreamer.queuePacket(parityPacket);
    parityPacket = null;
    return pstreamer.getLastQueuedSeqno();
  }
  ...
  static LocatedBlock addHybridBlock(DatanodeInfo[] excludedNodes,
                                     DFSClient dfsClient, String src, ExtendedBlock prevBlock,
                                     ExtendedBlock prevParity, ExtendedBlock prevReplica, boolean parityEnabled,
                                     long fileId, String[] favoredNodes, EnumSet<AddBlockFlag> allocFlags)
      throws IOException {
    final DfsClientConf conf = dfsClient.getConf();
    int retries = conf.getNumBlockWriteLocateFollowingRetry();
    long sleeptime = conf.getBlockWriteLocateFollowingInitialDelayMs();
    ...
    while (true) {
      try {
        return dfsClient.namenode.addHybridBlock(src, dfsClient.clientName,
            prevBlock, prevParity, prevReplica, parityEnabled, excludedNodes, fileId, favoredNodes, allocFlags);
      } catch (RemoteException e) {
        IOException ue = e.unwrapRemoteException(FileNotFoundException.class,
            AccessControlException.class,
            NSQuotaExceededException.class,
  ...
  static LocatedBlock[] addBlocks(DatanodeInfo[] excludedNodes,
                               DFSClient dfsClient, String src, ExtendedBlock prevBlock,
                               ExtendedBlock[] previousParities, ExtendedBlock partialParity, long fileId,
                               String[] favoredNodes, EnumSet<AddBlockFlag> allocFlags, int numStripes, int numGroups)
    throws IOException {
    final DfsClientConf conf = dfsClient.getConf();
    int retries = conf.getNumBlockWriteLocateFollowingRetry();
    long sleeptime = conf.getBlockWriteLocateFollowingInitialDelayMs();
    ...
    while (true) {
      try {
        return dfsClient.namenode.addBlocks(src, dfsClient.clientName,
              prevBlock, previousParities, partialParity, excludedNodes,
              fileId, favoredNodes, allocFlags, numStripes, numGroups);
      } catch (RemoteException e) {
        IOException ue = e.unwrapRemoteException(FileNotFoundException.class,
          AccessControlException.class,
          NSQuotaExceededException.class,
