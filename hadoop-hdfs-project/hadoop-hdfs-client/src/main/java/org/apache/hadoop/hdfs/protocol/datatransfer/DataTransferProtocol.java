...
public interface DataTransferProtocol {
  ...
  /**
   * Similar to writeBlock above but performs striped writing
   * at the end of the normal replicated pipeline.
   *
   * @param block replicated block
   * @param storageType current replicated storage type
   * @param storageId current replicated storage id
   * @param blockToken security token for accessing the block.
   * @param clientname client's name.
   * @param replicaNodes other downstream datanodes in the pipeline.
   * @param replicaStorageTypes target {@link StorageType}s corresponding
   *                            to the target datanodes.
   * @param replicaStorageIds optional StorageIDs designating where to write the block
   * @param stripedBlock striped block group id/size
   * @param stripedNodes sbg data node locations
   * @param stripedStorageTypes sbg storage types
   * @param stripedStorageIds sbg storage ids similar to replicaStorageIds
   * @param stripedOffsets block offsets of each striped node (may remove later)
   * @param srcDataNode source datanode
   * @param stage pipeline stage
   * @param pipelineSize size of the pipeline
   * @param minBytesRcvd min number of bytes received
   * @param maxBytesRcvd max number of bytes received
   * @param latestGenerationStamp the latest generation stamp of the block.
   * @param requestedChecksum the requested checksum mechanism
   * @param cachingStrategy the caching strategy
   * @param allowLazyPersist hint to the DataNode that the block can be flushed lazily
   * @throws IOException when it breaks
   */
  void writeHybridBlock(final ExtendedBlock block,
      final Token<BlockTokenIdentifier> blockToken,
      final String clientName,
      final StorageType storageType,
      final String storageId,
   ...
   *                  replaced to.
   * @param offset offset in the block to start replacing from
   */
   ...
   * @param blockToken security token for accessing the block.
   * @param offset offset in the block to start copying from
   */
