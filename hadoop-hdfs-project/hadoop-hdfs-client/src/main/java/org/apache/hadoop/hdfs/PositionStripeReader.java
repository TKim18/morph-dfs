...
class PositionStripeReader extends StripeReader {
  ...
  boolean prepareParityChunk(int i) {
    // index is the position within the erasure-coded stripe
    // while pos is the actual position within the bytebuffers
    int index = dataBlkNum + i;
    int pos = actualDataWidth + i;
    Preconditions.checkState(index >= dataBlkNum &&
        alignedStripe.chunks[pos] == null);

    int bufLen = (int) alignedStripe.getSpanInBlock();
    decodeInputs[index] = new ECChunk(codingBuffer.duplicate(), index * bufLen,
        bufLen);

    alignedStripe.chunks[pos] =
