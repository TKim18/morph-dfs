...
public class DFSClient implements java.io.Closeable, RemotePeerFactory,
  ...
  private static volatile ThreadPoolExecutor HYBRID_READ_THREAD_POOL;
  ...
  public DFSClient(URI nameNodeUri, ClientProtocol rpcNamenode,
    ...
    this.initThreadsNumForHybridReads();
  ...
  /**
   * Create an input stream that obtains the complete nodelist from
   * the namenode, specifically used in the case for erasure-coded files
   * which want to stripe together logical blocks into redundancy groups
   */
  public DFSInputStream openComplete(String src, int buffersize, boolean verifyChecksum)
          throws IOException {
    checkOpen();
    //    Get block info from namenode
    LOG.info("Calling open type 2: {}", src);
    try (TraceScope ignored = newPathTraceScope("newDFSInputStream", src)) {
      LocatedBlocks locatedBlocks = getLocatedFileInfo(src, true).getLocatedBlocks();
      LOG.info("Getting located blocks: {}", locatedBlocks);
      return openInternal(locatedBlocks, src, verifyChecksum);
    }
  }
  ...
  private DFSInputStream openInternal(LocatedBlocks locatedBlocks, String src,
    ...
    if (locatedBlocks != null) {
      ...
      if (lastBlock != null && lastBlock.isHybrid()) {
        return new DFSHybridInputStream(this, src, verifyChecksum, ecPolicy,
            locatedBlocks);
      } else if (ecPolicy != null) {
        LOG.info("Located blocks shows stripe width as = {}", ecPolicy.getStripeWidth());
   ...
   */
  public LastBlockWithStatus callAppend(String src,
 ...
 /**
  ...
  private static synchronized void initThreadsNumForHedgedReads(int num) {
    ...
    HEDGED_READ_THREAD_POOL = new ThreadPoolExecutor(num, num, 60,
  ...
  private void initThreadsNumForHybridReads() {
    if (HYBRID_READ_THREAD_POOL != null) {
      return;
    }
    synchronized (DFSClient.class) {
      if (HYBRID_READ_THREAD_POOL == null) {
        // Only after thread pool is fully constructed then save it to
        // volatile field.
        ThreadPoolExecutor threadPool = DFSUtilClient.getThreadPoolExecutor(2,
            2, 60, "HybridRead-", true);
        threadPool.allowCoreThreadTimeOut(true);
        HYBRID_READ_THREAD_POOL = threadPool;
      }
    }
  }
  ...
  ThreadPoolExecutor getHybridReadsThreadPool() {
    return HYBRID_READ_THREAD_POOL;
  }
