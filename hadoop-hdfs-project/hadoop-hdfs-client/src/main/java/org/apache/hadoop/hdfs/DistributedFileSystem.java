...
public class DistributedFileSystem extends FileSystem
  ...
  public boolean rename(Path src, Path dst) throws IOException {
    ...
    try {
      if (dst.toString().startsWith("transcode")) {
        // hijack this to call ecpolicy change
        String[] parts = dst.toString().split("_");
        if (parts.length == 2) {
          dfs.setErasureCodingPolicy(src.toString(), parts[1]);
          return true;
        } else {
          return false;
        }
