...
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.AddHybridBlockRequestProto;
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.AddBlocksRequestProto;
...
import org.apache.hadoop.hdfs.protocol.proto.ErasureCodingProtos.SetStripeWidthRequestProto;
import org.apache.hadoop.hdfs.protocol.proto.ErasureCodingProtos.SetStripeWidthResponseProto;
...
public class ClientNamenodeProtocolTranslatorPB implements
  ...
  @Override
  public LocatedBlock addHybridBlock(String src, String clientName,
                               ExtendedBlock previous, ExtendedBlock previousParity,
                               ExtendedBlock previousReplica, boolean parityEnabled, DatanodeInfo[] excludeNodes, long fileId,
                               String[] favoredNodes, EnumSet<AddBlockFlag> addBlockFlags)
      throws IOException {
    AddHybridBlockRequestProto.Builder req = AddHybridBlockRequestProto.newBuilder()
        .setSrc(src).setClientName(clientName).setFileId(fileId).setParityEnabled(parityEnabled);
    if (previous != null)
      req.setPrevious(PBHelperClient.convert(previous));
    if (previousParity != null)
      req.setPreviousParity(PBHelperClient.convert(previousParity));
    if (previousReplica != null)
      req.setPreviousReplica(PBHelperClient.convert(previousReplica));
    if (excludeNodes != null)
      req.addAllExcludeNodes(PBHelperClient.convert(excludeNodes));
    if (favoredNodes != null) {
      req.addAllFavoredNodes(Arrays.asList(favoredNodes));
    ...
    try {
      return PBHelperClient.convertLocatedBlockProto(
          rpcProxy.addHybridBlock(null, req.build()).getBlock());
    } catch (ServiceException e) {
      throw ProtobufHelper.getRemoteException(e);
    }
  }
  ...
  @Override
  public LocatedBlock[] addBlocks(String src, String clientName,
       ExtendedBlock previousData, ExtendedBlock[] previousParities,
       ExtendedBlock partialParity, DatanodeInfo[] excludeNodes, long fileId,
       String[] favoredNodes, EnumSet<AddBlockFlag> addBlockFlags,
       int numStripes, int numGroups)
    throws IOException {
    AddBlocksRequestProto.Builder req = AddBlocksRequestProto.newBuilder()
      .setSrc(src).setClientName(clientName).setFileId(fileId);
    if (previousData != null)
      req.setPreviousData(PBHelperClient.convert(previousData));
    if (previousParities != null)
      req.addAllPreviousParities(PBHelperClient.convert(previousParities));
    if (partialParity != null)
      req.setPartialParity(PBHelperClient.convert(partialParity));
    if (excludeNodes != null)
      req.addAllExcludeNodes(PBHelperClient.convert(excludeNodes));
    if (favoredNodes != null) {
      req.addAllFavoredNodes(Arrays.asList(favoredNodes));
    ...
    req.setNumStripes(numStripes);
    req.setNumGroups(numGroups);
    try {
      List<HdfsProtos.LocatedBlockProto> resp = rpcProxy.addBlocks(null, req.build()).getBlockList();
      return PBHelperClient.convertLocatedBlocks(resp.toArray(new HdfsProtos.LocatedBlockProto[0]));
    } catch (ServiceException e) {
      throw ProtobufHelper.getRemoteException(e);
    }
  }
  ...
  public boolean complete(String src, String clientName,
      ExtendedBlock last, ExtendedBlock[] parities, long fileId) throws IOException {
    CompleteRequestProto.Builder req = CompleteRequestProto.newBuilder()
        .setSrc(src)
        .setClientName(clientName)
        .addAllParities(PBHelperClient.convert(parities))
        .setFileId(fileId);
    if (last != null)
  ...
  @Override
  public void setStripeWidth(String src, int stripeWidth)
    throws IOException {
    final SetStripeWidthRequestProto.Builder builder =
      SetStripeWidthRequestProto.newBuilder();
    builder.setSrc(src);
    builder.setStripeWidth(stripeWidth);
    SetStripeWidthRequestProto req = builder.build();
    try {
      rpcProxy.setStripeWidth(null, req);
    } catch (ServiceException e) {
      throw ProtobufHelper.getRemoteException(e);
    }
  }
