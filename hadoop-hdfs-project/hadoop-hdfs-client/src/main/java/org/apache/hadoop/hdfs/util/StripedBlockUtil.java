...
public class StripedBlockUtil {
  ...
  public static LocatedBlock[] parseStripedBlockGroupWithParities(
      LocatedStripedBlock data, LocatedStripedBlock parity, int cellSize, int dataBlkNum, int parityBlkNum) {
    LocatedBlock[] lbs = new LocatedBlock[dataBlkNum + parityBlkNum];
    short idx = 0;
    for (; idx < dataBlkNum; idx++) {
      if (lbs[idx] == null) {
        lbs[idx] = constructInternalBlock(data, idx, cellSize, dataBlkNum, idx);
      }
    }
    if (parity != null) {
      for (; idx < dataBlkNum + parityBlkNum; idx++) {
        if (lbs[idx] == null) {
          int pos = idx - dataBlkNum;
          lbs[idx] = constructInternalBlock(parity, pos, cellSize, parityBlkNum, pos);
        }
      }
    }
    return lbs;
  ...
  /**
   * Parses multiple stripes part of a group into a list of located blocks.
   * @param dataBlkNum is the stripe width and not the group width
   */
  public static LocatedBlock[] parseStripedBlockGroupsWithParities(
      LocatedStripedBlock[] stripes, LocatedStripedBlock parity, int cellSize, int dataBlkNum, int parityBlkNum) {
    LocatedBlock[] lbs = new LocatedBlock[stripes.length * dataBlkNum + parityBlkNum];
    short idx, globalIdx = 0;
    for (LocatedStripedBlock stripe : stripes) {
      for (idx = 0; idx < dataBlkNum; idx++, globalIdx++) {
        if (lbs[globalIdx] == null) {
          lbs[globalIdx] = constructInternalBlock(stripe, idx, cellSize, dataBlkNum, idx);
        }
      }
    }

    if (parity != null) {
      for (idx = 0; idx < parityBlkNum; idx++, globalIdx++) {
        if (lbs[globalIdx] == null) {
          lbs[globalIdx] = constructInternalBlock(parity, idx, cellSize, parityBlkNum, idx);
        }
      }
    }
    return lbs;
  ...
  public static DatanodeInfo[] parseDataBlockLocations(LocatedStripedBlock bg, int dataBlkNum) {
    int locatedBGSize = bg.getBlockIndices().length;
    DatanodeInfo[] locs = bg.getLocations();
    List<DatanodeInfo> dns = new ArrayList<>();

    for (short i = 0; i < locatedBGSize; i++) {
      final int idx = bg.getBlockIndices()[i];
      // only data blocks
      if (idx < dataBlkNum) {
        // pick the location in the original location list that's at index i
        dns.add(locs[i]);
      }
    }
    return dns.toArray(DatanodeInfo.EMPTY_ARRAY);
  }
  ...
  public static ExtendedBlock constructInternalBlock(
    ...
    return constructInternalBlock(blockGroup, ecPolicy.getCellSize(),
        ecPolicy.getStripeWidth(), idxInBlockGroup);
  }
  /**
  ...
  public static long getSafeLength(ErasureCodingPolicy ecPolicy,
    ...
    final int dataBlkNum = ecPolicy.getStripeWidth();
...
//    DFSClient.LOG.info("Buffer offset: {}, rangeStartInBlockGroup: {}", bufOffset, rangeStartInBlockGroup);
  ...
  private static VerticalRange[] getRangesForInternalBlocks(
    ...
    int dataBlkNum = ecPolicy.getStripeWidth();
  ...
  private static AlignedStripe[] mergeRangesForInternalBlocks(
    ...
    int dataBlkNum = Math.max(ecPolicy.getNumDataUnits(), ecPolicy.getStripeWidth());
    int parityBlkNum = ecPolicy.getNumParityUnits();
    ...
    int idxInInternalBlk = lastCellIdxInBG / ecPolicy.getStripeWidth();
  ...
  public static class StripingCell {
    ...
    StripingCell(ErasureCodingPolicy ecPolicy, int cellSize, long idxInBlkGroup,
      ...
      this.idxInInternalBlk = idxInBlkGroup / ecPolicy.getStripeWidth();
      this.idxInStripe = (int)(idxInBlkGroup -
          this.idxInInternalBlk * ecPolicy.getStripeWidth());
      this.offset = offset;
      this.size = cellSize;
    }
