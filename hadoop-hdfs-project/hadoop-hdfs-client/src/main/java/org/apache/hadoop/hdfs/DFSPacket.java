...
public class DFSPacket {
  ...
  private final boolean lastPacketInStripe; // is this the last packet in the stripe block?
   ...
   * @param lastPacketInStripe if this is the last packet in stripe block group
   */
  public DFSPacket(byte[] buf, int chunksPerPkt, long offsetInBlock, long seqno,
                   int checksumSize, boolean lastPacketInBlock, boolean lastPacketInStripe) {
  ...
  public synchronized void writeTo(DataOutputStream stm) throws IOException {
    ...
    PacketHeader header = new PacketHeader(
        pktLen, offsetInBlock, seqno, lastPacketInBlock, lastPacketInStripe, dataLen, syncBlock);
  ...
  /**
   * check if this packet is the last packet in stripe block group
   *
   * @return true if above
   */
  boolean isLastPacketInStripe() {
    return lastPacketInStripe;
  }
