...
public class PBHelperClient {
  ...
  public static List<ExtendedBlockProto> convert(final ExtendedBlock[] ebs) {
    if (ebs == null) return null;
    List<ExtendedBlockProto> ebps = new ArrayList<>();
    for (ExtendedBlock eb : ebs) {
      ebps.add(convert(eb));
    }
    return ebps;
  }

  public static ExtendedGroupProto convert(final ExtendedGroup g) {
    if (g == null) return null;
    ExtendedBlock b = g.getBlock();

    if (b == null) return null;
    ExtendedGroupProto.Builder builder = ExtendedGroupProto.newBuilder().setBlock(convert(b));
    if (g.getBlockIdOffsets() != null) {
      for (int offset : g.getBlockIdOffsets()) {
        builder.addBlockIdOffsets(offset);
      }
    }
    if (g.getBlockLengths() != null) {
      for (long length : g.getBlockLengths()) {
        builder.addBlockLengths(length);
      }
    }
  ...
  public static List<Long> convert(Long[] ls, int idx) {
    List<Long> longs = new ArrayList<>();
    if (ls != null) {
      for (; idx < ls.length; ++idx) {
        longs.add(ls[idx]);
      }
    }
    return longs;
  ...
  public static ExtendedGroup convert(ExtendedGroupProto eg) {
    if (eg == null) return null;
    int[] blockIdOffsets = null;
    int blockOffsetCount = eg.getBlockIdOffsetsCount();
    if (blockOffsetCount != 0) {
      blockIdOffsets = new int[blockOffsetCount];
      for (int i = 0; i < blockOffsetCount; i++) {
        blockIdOffsets[i] = eg.getBlockIdOffsets(i);
      }
    }
    long[] blockLengths = null;
    int blockLengthsCount = eg.getBlockLengthsCount();
    if (blockLengthsCount != 0) {
      blockLengths = new long[blockLengthsCount];
      for (int i = 0; i < blockLengthsCount; i++) {
        blockLengths[i] = eg.getBlockLengths(i);
      }
    }
    return new ExtendedGroup(convert(eg.getBlock()),
      blockIdOffsets, blockLengths);
  }
  ...
  public static ExtendedBlock[] convertExtendedBlocks(List<ExtendedBlockProto> ebs) {
    if (ebs == null || ebs.isEmpty()) return null;
    ExtendedBlock[] r = new ExtendedBlock[ebs.size()];
    for (int i = 0; i < ebs.size(); i++) {
      r[i] = convert(ebs.get(i));
    }
    return r;
  }
  ...
  public static LocatedBlock convertLocatedBlockProto(LocatedBlockProto proto) {
    ...
    if (indices == null) {
    ...
    } else if (!proto.getReplicasList().isEmpty()) {
      lb = new LocatedHybridBlock(PBHelperClient.convert(proto.getB()),
          PBHelperClient.convert(proto.getParity()),
          PBHelperClient.convertExtendedBlocks(proto.getReplicasList()), targets,
      ...
      ((LocatedHybridBlock) lb).setBlockTokens(blockTokens);
    } else {
  ...
  public static LocatedBlocks convert(LocatedBlocksProto lb) {
    return new LocatedBlocks(
        ...
        convertLocatedBlocks(lb.getParitiesList()),
        lb.hasLastBlock() ?
  ...
  public static LocatedBlockProto convertLocatedBlock(LocatedBlock b) {
    ...
    if (b instanceof LocatedHybridBlock) {
      LocatedHybridBlock sb = (LocatedHybridBlock) b;
      byte[] indices = sb.getBlockIndices();
      builder.setBlockIndices(PBHelperClient.getByteString(indices));
      Token<BlockTokenIdentifier>[] blockTokens = sb.getBlockTokens();
      builder.addAllBlockTokens(convert(blockTokens));
      builder.addAllReplicas(convert(sb.getReplicas()));
      if (sb.getParity() != null) {
        builder.setParity(PBHelperClient.convert(sb.getParity()));
      }
    }
    ...
    for (HdfsFileStatusProto.Flags pbf : HdfsFileStatusProto.Flags.values()) {
      if ((pbf.getNumber() & flags) != 0) {
        ...
        case HAS_EC:
          ...
          break;
        case HAS_HYBRID:
          f.add(HdfsFileStatus.Flags.HAS_HYBRID);
          break;
  ...
  public static BlockTypeProto convert(BlockType blockType) {
    ...
    case PARITY: return BlockTypeProto.PARITY;
    case GROUPED: return BlockTypeProto.GROUPED;
    default:
  ...
  public static BlockType convert(BlockTypeProto blockType) {
    ...
    case BlockTypeProto.PARITY_VALUE: return BlockType.PARITY;
    case BlockTypeProto.GROUPED_VALUE: return BlockType.GROUPED;
    default:
      throw new IllegalArgumentException("Unexpected block type: " + blockType);
    }
  }
  ...
  public static LocatedBlocksProto convert(LocatedBlocks lb) {
    ...
    return builder.setFileLength(lb.getFileLength())
        ...
        .addAllParities(convertLocatedBlocks2(lb.getParityBlocks()))
  ...
  public static HdfsFileStatusProto convert(HdfsFileStatus fs) {
    ...
    flags |= fs.isHybridProtected() ? HdfsFileStatusProto.Flags.HAS_HYBRID_VALUE : 0;
  ...
  public static ErasureCodingPolicy convertErasureCodingPolicy(
    ...
    if (policy == null) {
      ...
      policy = new ErasureCodingPolicy(proto.getName(),
          convertECSchema(proto.getSchema()),
          proto.getStripeWidth(),
          proto.getCellSize(), id);
    } else {
      policy.setStripeWidth(proto.getStripeWidth());
    }
  ...
  private static ErasureCodingPolicyProto.Builder createECPolicyProtoBuilder(
    ...
    if (SystemErasureCodingPolicies.getByID(policy.getId()) == null) {
      builder.setName(policy.getName())
          ...
          .setStripeWidth(policy.getStripeWidth())
          .setCellSize(policy.getCellSize());
    } else {
      // manually set stripe width since that's not included
      builder.setStripeWidth(policy.getStripeWidth());
    }
    return builder;
  }
