...
public class DFSInputStream extends FSInputStream
  ...
  protected boolean isFinalRequest = false;
  ...
  protected void checkInterrupted(IOException e) throws IOException {
  ...
  protected DNAddrPair chooseDataNode(LocatedBlock block,
  ...
  private void hedgedFetchBlockByteRange(LocatedBlock block, long start,
    ...
    boolean triedSecond = false;
    Future<ByteBuffer> firstRequest = null, secondRequest = null, lastRequest = null;
    while (true) {
      ...
      if (firstTry) {
        ...
        try {
          ...
          if (chosenNode != null) {
            ...
            if (!triedSecond) {
              triedSecond = true;
              secondRequest = hedgedService.submit(getFromDataNodeCallable);
              ...
              // final try
              lastRequest = hedgedService.submit(getFromDataNodeCallable);
              futures.add(lastRequest);
              DFSClient.LOG.info("Spawning last read to replica {} after {} ms",
                  chosenNode.info, System.currentTimeMillis() - startTime);
            }
          }
        } catch (IOException ioe) {
        ...
        try {
          ...
          if (future != null) {
            ...
            if (future == firstRequest) {
              DFSClient.LOG.info("Successfully read from first replica in {} ms",
                  System.currentTimeMillis() - startTime);
            } else if (future == secondRequest) {
              DFSClient.LOG.info("Successfully read from second replica in {} ms",
                  System.currentTimeMillis() - startTime);
            } else if (future == lastRequest){
              DFSClient.LOG.info("Successfully read from third replica in {} ms",
                  System.currentTimeMillis() - startTime);
            } else {
  ...
  public int read(long position, final ByteBuffer buf) throws IOException {
    DFSClient.LOG.info("Calling read at pos {}", position);
    if (!buf.hasRemaining()) {
      return 0;
    }
  ...
  public void readFully(long position, final ByteBuffer buf)
      throws IOException {
    int nread = 0;
    DFSClient.LOG.info("Calling readfully at pos {}", position);
