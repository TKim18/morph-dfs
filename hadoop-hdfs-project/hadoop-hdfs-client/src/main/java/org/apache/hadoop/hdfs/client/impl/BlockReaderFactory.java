...
public class BlockReaderFactory implements ShortCircuitReplicaCreator {
  ...
  private BlockReader getRemoteBlockReaderFromTcp() throws IOException {
    ...
    while (true) {
      ...
      try {
        ...
        curPeer = nextTcpPeer();
        LOG.trace("Took {} ms to make tcp peer",
                System.currentTimeMillis() - startTime);
        if (curPeer.fromCache) remainingCacheTries--;
        peer = curPeer.peer;
        startTime = System.currentTimeMillis();
        blockReader = getRemoteBlockReader(peer);
        LOG.trace("Took {} ms to make remote block reader",
                System.currentTimeMillis() - startTime);
        return blockReader;
      } catch (IOException ioe) {
        if (isSecurityException(ioe)) {
  ...
  private BlockReaderPeer nextTcpPeer() throws IOException {
    if (remainingCacheTries > 0) {
      ...
      if (peer != null) {
        LOG.trace("nextTcpPeer: reusing existing peer");
        return new BlockReaderPeer(peer, true);
      }
    }
    try {
      Peer peer = remotePeerFactory.newConnectedPeer(inetSocketAddress, token,
          datanode);
      LOG.trace("nextTcpPeer: created newConnectedPeer");
