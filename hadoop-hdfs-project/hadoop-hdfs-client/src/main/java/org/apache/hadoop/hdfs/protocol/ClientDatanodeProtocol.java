...
public interface ClientDatanodeProtocol {
  ...
  /**
   * Trigger data movement from a block at a cell-level granularity
   *
   * @param block block replica to read data from
   * @param offset offset of the block to move data from
   * @param numBytes number of bytes to read off the block
   */
  void triggerCellMovement(ExtendedBlock block, long offset, long numBytes) throws IOException;
}
