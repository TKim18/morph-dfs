...
int update(IsalEncoder* pCoder, unsigned char** dataUnits,
    unsigned char** parityUnits, int offset, int length, int chunkSize) {
  // group offset is the starting position of the data units relative within the group
  int numDataUnits = pCoder->coder.numDataUnits;
  int numParityUnits = pCoder->coder.numParityUnits;

  // loop over each array of data units until offset == length, use offset as init vector index
  for (int i = 0; i < length; i++) {
     h_ec_encode_data_update(chunkSize, numDataUnits, numParityUnits,
                            i + offset, pCoder->gftbls, dataUnits[i], parityUnits);
  }
