...
JNIEXPORT void JNICALL
Java_org_apache_hadoop_io_erasurecode_rawcoder_NativeRSRawEncoder_updateImpl(
JNIEnv *env, jobject thiz, jobjectArray inputs, jintArray inputOffsets,
jint dataLen, jobjectArray outputs, jintArray outputOffsets, jint offset, jint length) {
  RSEncoder* rsEncoder = (RSEncoder*)getCoder(env, thiz);
  if (!rsEncoder) {
    THROW(env, "java/io/IOException", "NativeRSRawEncoder closed");
    return;
  ...
  getInputs(env, inputs, inputOffsets, rsEncoder->inputs, length);
  getOutputs(env, outputs, outputOffsets, rsEncoder->outputs, numParityUnits);

  update(&rsEncoder->encoder, rsEncoder->inputs, rsEncoder->outputs, offset, length, chunkSize);
}
