...
public final class CodecUtil {
  ...
  public static final String IO_ERASURECODE_CODEC_CC_KEY = IO_ERASURECODE_CODEC + "cc";

  public static final String IO_ERASURECODE_CODEC_CC =
          CCErasureCodec.class.getCanonicalName();
  ...
  /** Erasure coder Bandwidth-Optimal Convertible Code codec.*/
  public static final String IO_ERASURECODE_CODEC_BCC_KEY =
          IO_ERASURECODE_CODEC + "bcc";
  public static final String IO_ERASURECODE_CODEC_BCC =
          BCCErasureCodec.class.getCanonicalName();

  /** Erasure coder Local Reconstructions Convertible Code codec.*/
  ...
  public static RawErasureTranscoder createRawTranscoder(
          Configuration conf, String codec, TranscoderOptions coderOptions) {
    Preconditions.checkNotNull(conf);
    Preconditions.checkNotNull(codec);

    return createRawTranscoderWithFallback(conf, codec, coderOptions);
  }
  ...
  private static RawErasureTranscoder createRawTranscoderWithFallback(
          Configuration conf, String codecName, TranscoderOptions coderOptions) {
    String[] rawCoderNames = getRawCoderNames(conf, codecName);
    for (String rawCoderName : rawCoderNames) {
      try {
        if (rawCoderName != null) {
          RawErasureCoderFactory fact = createRawCoderFactory(
                  rawCoderName, codecName);
          return fact.createTranscoder(coderOptions);
        }
      } catch (LinkageError | Exception e) {
        // Fallback to next coder if possible
        if (LOG.isDebugEnabled()) {
          LOG.debug("Failed to create raw erasure transcoder " + rawCoderName +
                  ", fallback to next codec if possible", e);
        }
      }
    }
    throw new IllegalArgumentException("Fail to create raw erasure " +
            "transcoder with given codec: " + codecName);
  }
  ...
  private static String getCodecClassName(Configuration conf, String codec) {
    switch (codec) {
    case ErasureCodeConstants.CC_CODEC_NAME:
      return conf.get(
              CodecUtil.IO_ERASURECODE_CODEC_CC_KEY,
              CodecUtil.IO_ERASURECODE_CODEC_CC);
    ...
    case ErasureCodeConstants.BCC_CODEC_NAME:
      return conf.get(
          CodecUtil.IO_ERASURECODE_CODEC_BCC_KEY,
          CodecUtil.IO_ERASURECODE_CODEC_BCC);
