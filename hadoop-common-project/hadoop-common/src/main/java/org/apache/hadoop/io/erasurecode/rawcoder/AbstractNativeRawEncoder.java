...
abstract class AbstractNativeRawEncoder extends RawErasureEncoder {
  ...
  protected void doUpdate(ByteBufferUpdatingState updatingState)
    ...
    try {
      ...
      int[] inputOffsets = new int[updatingState.inputs.length];  // length should be 1
      int[] outputOffsets = new int[updatingState.outputs.length];
      int dataLen = updatingState.inputs[0].remaining();

      ByteBuffer buffer;
      for (int i = 0; i < updatingState.inputs.length; ++i) {
        buffer = updatingState.inputs[i];
        inputOffsets[i] = buffer.position();
      }

      for (int i = 0; i < updatingState.outputs.length; ++i) {
        buffer = updatingState.outputs[i];
        outputOffsets[i] = buffer.position();
      }

      performUpdateImpl(updatingState.inputs, inputOffsets, dataLen,
        updatingState.outputs, outputOffsets,
        updatingState.offsetInGroup, updatingState.length);
    } finally {
      encoderLock.readLock().unlock();
    }
  }

  protected abstract void performUpdateImpl(
    ByteBuffer[] inputs, int[] inputOffsets,
    int dataLen, ByteBuffer[] outputs,
    int[] outputOffsets,
    int offset, int length)  // offset here refers to position within group
    throws IOException;
