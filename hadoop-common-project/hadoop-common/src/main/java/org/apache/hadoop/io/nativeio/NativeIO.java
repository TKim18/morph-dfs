...
public class NativeIO {
  public static class POSIX {
    ...
    // Flags for fallocate() call from bits/fcntl.h - Set by JNI
    public static int FALLOC_FL_KEEP_SIZE = 1;
    public static int FALLOC_FL_PUNCH_HOLE = 2;
    public static int FALLOC_FL_UNSHARE_RANGE = -1;
    public static int FALLOC_FL_COLLAPSE_RANGE = -1; // by default is 8
    public static int FALLOC_FL_ZERO_RANGE = -1;
    ...
    /** Wrapper around fallocate(2) */
    static native void fallocate(
      FileDescriptor fd, int mode, long offset, long len) throws NativeIOException;
    ...
    /**
     * Call fallocate on the given file descriptor. See the manpage
     * for this syscall for more information. On systems where this
     * call is not available, does nothing.
     *
     * @throws NativeIOException if there is an error with the syscall
     */
    public static void fallocateIfPossible(
        FileDescriptor fd, int mode, long offset, long nbytes)
        throws NativeIOException {
      if (nativeLoaded && fallocatePossible) {
        try {
          fallocate(fd, mode, offset, nbytes);
        } catch (UnsupportedOperationException | UnsatisfiedLinkError ue) {
          fallocatePossible = false;
          throw new NativeIOException(ue.getMessage(), ue.getCause());
        }
      } else {
        LOG.info("UNABLE TO LOAD because nativeloaded = {} and fallocatepossible = {}", nativeLoaded, fallocatePossible);
      }
    }
