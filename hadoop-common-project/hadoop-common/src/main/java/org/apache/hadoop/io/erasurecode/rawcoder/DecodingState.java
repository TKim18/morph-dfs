...
class DecodingState {
  ...
  <T> void checkParameters(T[] inputs, int[] erasedIndexes,
                           T[] outputs) {
    if (inputs.length != decoder.getNumParityUnits() + decoder.getNumLocalParityUnits() +
    ...
    if (erasedIndexes.length > decoder.getNumParityUnits() + decoder.getNumLocalParityUnits()) {
      throw new HadoopIllegalArgumentException(
