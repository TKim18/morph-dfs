...
public abstract class TestCoderBase {
  static final Logger LOG = LoggerFactory.getLogger(TestCoderBase.class);
  ...
  protected int numDataUnitsFinal = 0;
  protected  int numParityUnitsFinal = 0;
  protected  int numLocalParityUnits = 0;
   ...
   */
  protected void prepare(Configuration conf, int numDataUnits,
                         int numParityUnits, int numDataUnitsFinal,
                         int numParityUnitsFinal, int[] erasedDataIndexes,
                         int[] erasedParityIndexes, boolean usingFixedData) {
    this.conf = conf != null ? conf : new Configuration();
    this.numDataUnits = numDataUnits;
    this.numParityUnits = numParityUnits;
  ...
  protected void prepare(Configuration conf, int numDataUnits,
                         int numLocalParityUnits, int numParityUnits,
  ...
  protected void prepare(Configuration conf, int numDataUnits,
    ...
    prepare(conf, numDataUnits, numLocalParityUnits, numParityUnits,
            erasedDataIndexes, erasedParityIndexes, false);
  }
  ...
  protected ECChunk[] prepareInputChunksForDecoding(ECChunk[] dataChunks,
    ...
    ECChunk[] inputChunks = new ECChunk[numDataUnits + numLocalParityUnits + numParityUnits];
  ...
  protected ECChunk[] prepareParityChunksForEncoding() {
    ECChunk[] chunks = new ECChunk[numParityUnits + numLocalParityUnits];
    for (int i = 0; i < chunks.length; i++) {
  ...
  protected void dumpSetting() {
    if (allowDump) {
      ...
      sb.append(" numDataUnitsFinal=").append(numDataUnitsFinal);
      sb.append(" numParityUnitsFinal=").append(numParityUnitsFinal);
